...
```
    今天周六，早晨出门吃饭，全身汗湿透。天气真的是太热了！我决定一天不出门，在屋子里面休息！晚上，腾飞给我说了他暑假的计划，
决定去长沙、
```
...
文件名字叫做`read)utils.py`。该文件中实现了一个类`TextConverter`和一个工具函数`batch_generator`。
...
为来将所有数据都显示出来，我使用了TensorBoard进行显示。
