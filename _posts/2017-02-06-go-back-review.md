...
然后写出其对应的齐次方程，即根据含有$$T(n),T(n-1),T(n-2), \ldots ,T(n-k)$$的个数减去1来确定方程未知数的最大次数为$$k$$，然后将方程中的$$T(n)$$改为未知数的最大次数，$$T(n-1)$$改为未知数的第二大次数，以此类推，
假设齐次方程的未知数是$$t$$，则结果如下式：

$$
t^k-C_1t^{k-1}-C_2t^{k-2}- \cdots -C_k=f(n)
$$

根据该方程的形式，可以知道如果该方程的某个解$$t=r$$是$$m$$重根，则$$m$$个解的形式是$$\{ r^n,nr^n,n^2r^n, \ldots ,n^{m-1}r^n \}$$，如果剩余的解有多重跟也按照这种形式写出即可，为了简便，假设剩余的解都为单根，
则还剩$$(k-m)$$个解，其解的形式是$$\{ r^n_1,r^n_2, \ldots ,r^n_{k-m} \}$$，然后假设每个解的形式对应的系数分别为$$a_1,a_2, \ldots ,a_k$$，则可以写出齐次方程的通解形式：
...
|$$f(n)$$形式|条件|特解形式|
|--|--|--|
|$$a^n$$|$$C(a) \neq 0$$ <br>----------------------<br> $$a$$是$$C(t)$$的$$m$$重根|$$p_0a^n$$ <br>-----------<br> $$p_0n^ma^n$$|
|$$n^s$$|$$C(1) \neq 0$$ <br>----------------------<br> $$1$$是$$C(t)$$的$$m$$重根|$$p_0+p_1n+p_2n^2+\ldots +p_sn^s$$ <br>--------------------------------------------------<br> $$n^m(p_0+p_1n+p_2n^2+\ldots +p_sn^s)$$|
|$$n^sa^n$$|$$C(a) \neq 0$$ <br>----------------------<br> $$a$$是$$C(t)$$的$$m$$重根|$$(p_0+p_1n+p_2n^2+\ldots +p_sn^s)a^n$$ <br>----------------------------------------------------<br> $$n^m(p_0+p_1n+p_2n^2+\ldots +p_sn^s)a^n$$|
|常数|无|常数$$x$$|

注意：这个表格中需要求的未知数是：$$p_0,p_1, \ldots ,p_s,x$$，根据$$f(n)$$形式和条件在表中找到对应的特解形式———假设特解形式为$$M$$，令$$T(n)=M$$回代入到递归方程$$T(n)=C_1T(n-1)+C_2T(n-2)+\cdots+C_kT(n-k)+f(n)$$中，
求出未知数$$p_0,p_1, \ldots ,p_s,x$$，将这些未知数代回特解中，即得到特解的确定形式，设其确定形式是$$\zeta$$，则可以写出其次方程的“通解+特解”形式：
...
[python27的博客](http://www.cnblogs.com/python27/archive/2011/12/09/2282486.html,"Python21的博客")
